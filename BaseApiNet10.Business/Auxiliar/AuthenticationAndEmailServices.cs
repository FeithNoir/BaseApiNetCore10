...
namespace BaseApiNet10.Business.Auxiliar
{
    public static class AuthenticationAndEmailServices
    {
        public static void AddCustomAuthenticationAndEmailServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            .AddJwtBearer(jwt =>
            {
                var key = Encoding.ASCII.GetBytes(configuration.GetSection("JwtConfig:Secret").Value ?? throw new Exception("Error finding JWT Configuration."));
